...
/// if a log handler is set repeatedly. The alternative would be to use the `opaque` parameter from the libsrt logging
/// infrastructure. This approach may be desirable if this mutex becomes a performance problem. In that scenario we
/// would have no choice but to accept leaking. This may be acceptable.
static LOG_HANDLER: Mutex<Option<LogHandler>> = Mutex::new(None);

type LogHandler = Box<dyn FnMut(&SrtLogEvent) + Send + Sync + 'static>;
    ...
    F: FnMut(&SrtLogEvent) + Send + Sync + 'static,
{
    ...
    *lock = Some(closure);
    unsafe {
        srt_setloghandler(null_mut(), Some(log_handler_adapter));
    }
}
...
unsafe extern "C" fn log_handler_adapter(opaque: *mut c_void, level: c_int, file: *const c_char, line: c_int, area: *const c_char, message: *const c_char) {
    ...
    if r.is_err() {
        let _ = panic::catch_unwind(|| {
            eprintln!("srt log handler panicked many times");
        });
        // This runs afoul of https://github.com/rust-lang/rust/issues/86027 however an abort is less preferable than undefined behavior.
    }
}

fn log_handler_adapter_inner(_opaque: *mut c_void, level: c_int, file: *const c_char, line: c_int, area: *const c_char, message: *const c_char) {
    // The final panic layer is not very informative, so try and catch any panics possible here.
    let r = panic::catch_unwind(|| {
        let mut lock = LOG_HANDLER.lock().unwrap();
        if let Some(handler) = lock.as_mut() {
            let file_c_str = unsafe { cstr_to_cow_lossy(file) };
            let area_c_str = unsafe { cstr_to_cow_lossy(area) };
            let message_c_str = unsafe { cstr_to_cow_lossy(message) };
            (handler)(&SrtLogEvent {
    ...
    if let Err(e) = r {
        ...
        // the log macro can panic as well.
        let result_on_log = panic::catch_unwind(|| {
            if let Some(s) = &cow_panic {
                log::error!("srt log handler panicked: {s}");
            } else {
                log::error!("srt log handler panicked, panic value was not &str or String.");
            }
        });
        if let Err(e) = result_on_log {
            let log_panic = panic_to_cow_str(e).ok();
            // Because failing silently is considered a greater crime than using stderr from a library,
            // we will eprintln as a last resort. This is a likely outcome because if a log failed once it
            // very well could fail again.

            // Writing to io::stderr can also panic. If this occurs we've exhausted our options.
            let _ = panic::catch_unwind(|| {
                if let Some(s) = &cow_panic {
                ...
                // Now try and write the log panic as well.
                if let Some(s) = &log_panic {
                    eprintln!("panicked while using log error! macro: {s}");
                } else {
                    eprintln!("panicked while using log error! macro, panic value was not &str or String.");
                }
            });
        }
    }
