...
struct EncodedFrame {
    data_io: sys::ni_logan_session_data_io_t,
...
impl EncodedFrame {
    pub fn new() -> Result<Self> {
        ...
        Ok(Self {
            data_io: sys::ni_logan_session_data_io_t {
                data: sys::_ni_logan_session_data_io__bindgen_ty_1 { packet },
            },
            parameter_sets: None,
        })
    }
    ...
    pub fn as_slice(&self) -> &[u8] {
        ...
        &data[sys::NI_LOGAN_FW_ENC_BITSTREAM_META_DATA_SIZE as usize..]
    }
    ...
    pub fn packet(&self) -> &sys::ni_logan_packet_t {
    ...
    pub fn as_data_io_mut_ptr(&mut self) -> *mut sys::ni_logan_session_data_io_t {
...
impl Drop for EncodedFrame {
    fn drop(&mut self) {
        unsafe {
            sys::ni_logan_packet_buffer_free(&mut self.data_io.data.packet as _);
        }
    }
}
...
pub struct XcoderEncoder<F> {
    session: *mut sys::ni_logan_session_context_t,
    ...
    frame_data_io: sys::ni_logan_session_data_io_t,
...
impl<F> XcoderEncoder<F> {
    pub fn new(config: XcoderEncoderConfig) -> Result<Self> {
        ...
        unsafe {
            ...
            let session = sys::ni_logan_device_session_context_alloc_init();
            if session.is_null() {
                return Err(XcoderEncoderError::UnableToAllocateDeviceSessionContext);
            }
            let mut session = guard(session, |session| {
                sys::ni_logan_device_session_context_free(session);
            });

            (**session).hw_id = -1;
            (**session).p_session_config = &*params as *const sys::ni_logan_encoder_params_t as _;
            (**session).codec_format = match config.codec {
                XcoderEncoderCodec::H264 => sys::_ni_logan_codec_format_NI_LOGAN_CODEC_FORMAT_H264,
                XcoderEncoderCodec::H265 => sys::_ni_logan_codec_format_NI_LOGAN_CODEC_FORMAT_H265,
            };
            (**session).src_bit_depth = 8;
            (**session).src_endian = sys::NI_LOGAN_FRAME_LITTLE_ENDIAN as _;
            (**session).bit_depth_factor = 1;

            let code = sys::ni_logan_device_session_open(*session, sys::ni_logan_device_type_t_NI_LOGAN_DEVICE_TYPE_ENCODER);
            if code != sys::ni_logan_retcode_t_NI_LOGAN_RETCODE_SUCCESS {
                return Err(XcoderEncoderError::Unknown {
                    code,
            ...
            let did_finish = guard(false, |did_finish| {
                sys::ni_logan_device_session_close(
                    *session,
                    if did_finish { 1 } else { 0 },
                    sys::ni_logan_device_type_t_NI_LOGAN_DEVICE_TYPE_ENCODER,
                );
                sys::ni_logan_device_close((**session).device_handle);
                sys::ni_logan_device_close((**session).blk_io_handle);
            });
            ...
            let frame_data_io = guard(frame_data_io, |mut frame_data_io| {
                sys::ni_logan_frame_buffer_free(&mut frame_data_io.data.frame as _);
            });
...
impl<F: RawVideoFrame<u8>> XcoderEncoder<F> {
    fn try_reading_encoded_frames(&mut self) -> Result<()> {
        unsafe {
            while !self.did_finish {
                let code = sys::ni_logan_device_session_read(
                    self.session,
    ...
    fn try_write_frame(&mut self, f: F, force_key_frame: bool) -> Result<Option<F>> {
        ...
        self.did_start = true;
        let written =
            unsafe { sys::ni_logan_device_session_write(self.session, &mut self.frame_data_io, sys::ni_logan_device_type_t_NI_LOGAN_DEVICE_TYPE_ENCODER) };
        if written < 0 {
            return Err(XcoderEncoderError::Unknown {
                code: written,
                operation: "writing",
...
impl<F> Drop for XcoderEncoder<F> {
    fn drop(&mut self) {
        unsafe {
            sys::ni_logan_frame_buffer_free(&mut self.frame_data_io.data.frame as _);
            sys::ni_logan_device_session_close(
                self.session,
                if self.did_finish { 1 } else { 0 },
                sys::ni_logan_device_type_t_NI_LOGAN_DEVICE_TYPE_ENCODER,
            );
            sys::ni_logan_device_close((*self.session).device_handle);
            sys::ni_logan_device_close((*self.session).blk_io_handle);
            sys::ni_logan_device_session_context_free(self.session);
        }
    }
}
...
impl<F: RawVideoFrame<u8>> VideoEncoder for XcoderEncoder<F> {
    ...
    fn flush(&mut self) -> Result<Option<VideoEncoderOutput<F>>> {
        if !self.did_flush {
            let code = unsafe { sys::ni_logan_device_session_flush(self.session, sys::ni_logan_device_type_t_NI_LOGAN_DEVICE_TYPE_ENCODER) };
            if code != sys::ni_logan_retcode_t_NI_LOGAN_RETCODE_SUCCESS {
