...
/*                                                                           */
/*  List of Functions : ifwh264d_api_function                              */
/*                                                                           */
...
#define FW_CREATE_DUMP_FILE(m_filename)                                        \
    {                                                                          \
        ...
        {                                                                      \
            printf("Could not open file %s", m_filename);                      \
            printf("open fail errno = %d reason = %s \n", errno,               \
                   strerror(errno));                                           \
        }                                                                      \
    }

#define FW_DUMP_TO_FILE(m_filename, m_buf, m_size, m_offset)                   \
    {                                                                          \
        ...
        {                                                                      \
            ...
            printf("fwrite ret %d to write %d", i, m_size);                    \
            if (i != (int)m_size)                                              \
            {                                                                  \
                printf("Error in fwrite, returned %d", i);                     \
                perror("Error in write to file");                              \
            }                                                                  \
            if (fflush(fp))                                                    \
            {                                                                  \
                printf("Error in fflush, returned %d", i);                     \
            }                                                                  \
        } else if (fp == NULL)                                                 \
        {                                                                      \
            printf("Could not write to file %s", m_filename);                  \
            printf("write fail errno = %d reason = %s \n", errno,              \
                   strerror(errno));                                           \
        }                                                                      \
        if (fp)                                                                \
        {                                                                      \
...
/*****************************************************************************/
IV_API_CALL_STATUS_T ifwh264d_api_function(iv_obj_t *ps_handle, void *pv_api_ip,
                                           void *pv_api_op);
...
} ifwh264d_delete_ip_t;

typedef struct
{
    ivd_delete_op_t s_ivd_delete_op_t;
} ifwh264d_delete_op_t;
...
typedef struct
{
    ivd_fwcheck_ip_t s_ivd_fwcheck_ip_t;
} ifwh264d_fwcheck_ip_t;

typedef struct
{
    ivd_fwcheck_op_t s_ivd_fwcheck_op_t;
} ifwh264d_fwcheck_op_t;
...
} ifwh264d_create_ip_t;

typedef struct
{
    ivd_create_op_t s_ivd_create_op_t;
} ifwh264d_create_op_t;
...
} ifwh264d_video_decode_ip_t;

typedef struct
{
    ivd_video_decode_op_t s_ivd_video_decode_op_t;
} ifwh264d_video_decode_op_t;
...
typedef enum
{
    ...
    IFWH264D_CMD_CTL_SET_NUM_CORES = IVD_CMD_CTL_CODEC_SUBCMD_START,

    /** Set processor details */
    IH264D_CMD_CTL_SET_PROCESSOR = IVD_CMD_CTL_CODEC_SUBCMD_START + 0x001,

    /** Get display buffer dimensions */
    IH264D_CMD_CTL_GET_BUFFER_DIMENSIONS =
        IVD_CMD_CTL_CODEC_SUBCMD_START + 0x100,
...
} ifwh264d_ctl_set_num_cores_ip_t;

typedef struct
{
    UWORD32 u4_size;
    UWORD32 u4_error_code;
} ifwh264d_ctl_set_num_cores_op_t;
