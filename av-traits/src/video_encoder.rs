...
/// Implements basic video encoding behavior. Arbitrary data associated with each frame can be
...
/// fn encode<S, E: VideoEncoder<Box<dyn RawVideoFrame<u8>>>>(mut source: S, mut encoder: E) -> Result<(), E::Error>
...
pub trait VideoEncoder<F, C = ()> {
    ...
    /// frame.
    fn encode(&mut self, frame: VideoEncoderInput<F, C>) -> Result<Option<VideoEncoderOutput<C>>, Self::Error>;
    ...
    fn flush(&mut self) -> Result<Option<VideoEncoderOutput<C>>, Self::Error>;
}
...
mod test {
    ...
    fn test_video_encoder_object_safety() {
        let _e: *const dyn VideoEncoder<(), Error = ()>;
    }
}
